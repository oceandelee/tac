...
def download(urls, offset=0):
    ...
    for url in urls[offset:]:
        ...
        Path(os.path.dirname(os.path.abspath(__file__))+"\\..\\rapport\\data\\pdf").mkdir(parents=True, exist_ok=True)
        with open(os.path.dirname(os.path.abspath(__file__)) + f"\\..\\rapport\\data\\pdf\\{filename}", 'wb') as f:
...
def check(urls):
    ...
    for url in urls:
        filename = url.split("/")[-1]
        downloads = os.listdir(os.path.dirname(os.path.abspath(__file__)) +'\\..\\rapport\\data\\pdf')
